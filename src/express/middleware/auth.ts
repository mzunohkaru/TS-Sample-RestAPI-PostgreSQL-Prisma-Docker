...
export const authenticate = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    logger.debug('User authenticated successfully', { userId: payload.userId }, { requestId: req.headers['x-request-id'] as string });
    next();
  } catch (error) {
    if (error instanceof AppError) {
      ...
      logger.error('Unexpected authentication error', { error: error instanceof Error ? error.message : 'Unknown error' });
...
export const authorize = (...roles: string[]) => {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      logger.debug('User authorized successfully', { userId: authenticatedReq.user.userId, roles });
      next();
    } catch (error) {
...
export const optionalAuth = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    logger.warn('Optional authentication failed, proceeding without user context', { 
      error: error instanceof Error ? error.message : 'Unknown error' 
    });
    next();
  }
